...
- Non-functional: Not in scope for this assignment (no performance/security tests automated).
...
Risks & Assumptions
- The public demo site may reset or throttle accounts; tests designed to be resilient but may fail if site is unavailable.
...
- Automated tests run cleanly in CI (headless mode) and artifacts are produced.

Maintenance
- Keep Page Objects small and focused; add new pages for additional flows.
- Add data-driven tests for broader coverage.
- Integrate stronger reporting (e.g., ExtentReports) if richer visualization is required.
